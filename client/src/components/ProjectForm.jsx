    ...
    return(
        ...
        <div>
            <div className='flex items-center justify-between'>
                ...
                <button onClick={addProject} className='flex items-center gap-2 px-3 py-1 text-sm bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-100 transition-colors disabled:opacity-50'>
                    <Plus className='size-4' />
                    Add Project
                </button>
            </div>
            ...
            <div className='space-y-4 mt-6'>
                {data.map((project, index)=>(
                    <div key={index} className='p-4 border border-gray-200 space-y-3'>
                        ...
                        <div className='grid gap-3'>
                            <input value={project.name || ""} onChange={(e)=>updatedProject(index,"name", e.target.value)} type="text" placeholder='Project Name' className='px-3 py-2 text-sm rounded-none'/>
                            <input value={project.type || ""} onChange={(e)=>updatedProject(index,"type", e.target.value)} type="text" placeholder='Project Type' className='px-3 py-2 text-sm rounded-none'/>
                            <textarea rows={4} value={project.description || ""} onChange={(e)=>updatedProject(index,"description", e.target.value)} placeholder='Describe your project....' className='w-full px-3 py-2 text-sm resize-none rounded-none'/>
                        </div>
                    </div>
                ))}
            </div>
