import { User, Mail, Phone, MapPin, BriefcaseIcon, Linkedin, Globe } from 'lucide-react';
...
const PersonalInfoForm=({data,onChange,removeBackground,setRemoveBackground})=>{
    ...
    const fields=[
        ...
        {key:"profession", label:"Profession", icon:BriefcaseIcon, type:"text"},
    ...
    return(
        // <div>
        //     <h3 className='text-lg font-semibold text-gray-900'>Personal Informtion</h3>
        ...
        <div>
            ...
            <div className='flex items-center gap-2'>
                <label >
                    {data.image?(
                        <img src={typeof data.image==='string'? data.image : URL.createObjectURL(data.image)} alt="user-image" className='w-16 h-16 object-cover mt-5 ring ring-slate-300 hover:opacity-80'/>
                    ):(
                        <div className='inline-flex items-center gap-2 mt-5 text-slate-600 hover:text-slate-700 cursor-pointer'>
                            <User className='size-10 p-2.5 border rounded-full' />
                            Upload user image
                ...
                {typeof data.image==='object' && (
                    <div className='flex flex-col gap-1 pl-4 text-sm'>
                        ...
                        <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3' >
                            ...
                            <div className='w-9 h-5 bg-slate-300 peer peer-checked:bg-blue-600 transition-colors duration-200'>

                            </div>
                            <span className='dot absolute left-1 top-1 w-3 h-3 bg-white transition-transform duration-200 ease-in-out peer-checked:translate-x-4'></span>
                        </label>
                    </div>
                )}
            </div>
            ...
            {fields.map((field)=>{
                ...
                return(
                    <div key={field.key} className='space-y-1 mt-5' >
                        ...
                        <input type="{field.type}" value={data[field.key] || ""} onChange={(e)=>handleChange(field.key,e.target.value)} className='mt-1 w-full px-3 py-2 rounded-none border border-gray-300 focus:ring focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors text-sm' placeholder={`Enter your ${field.label.toLocaleLowerCase()}`} required={field.required} />
                    </div>
                )
            })}
